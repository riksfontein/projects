CH02EP17 - Omni Flash chain (ch02 ep17) - clips 1-102 of 102 (17 min 0 s)

1. FIRST FRAME  (only for clip 1)
   TurboFlow -> Mode Image -> Nano Banana Pro -> 16:9 -> Images per prompt x2 -> 2K Upscale
   Prompt: CH02EP17_omni_first_frame.txt    Save folder: ch02ep17-omni    prefix: ff  sep _  start 1
...
2. CHAIN
   ...
   Prompts: import CH02EP17_omni_chain.txt (102 prompts, each {...} block = 1 prompt)
   CHECK before Run: Queue tab row shows the "continuous" tag. No tag = not chained.
   Output: Downloads\ch02ep17-omni\clip_001.mp4 ... clip_102.mp4

3. After a stop / failures: Retry Failed keeps the numbering. A chain that broke mid-way resumes with
   --range K-102: its start frame is the last frame of the last good clip.

Clip k covers narration est CH02EP17_omni_clipmap.json -> est_window; the assembler retimes each clip onto the
